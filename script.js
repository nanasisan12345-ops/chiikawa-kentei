...
function home() {
  view = "home";
  mount(
    `<section class="hero"><div><span class="tag">草むしりの、その先のひと休み。</span><p class="eyebrow">A LITTLE EXAM, A LOT OF LOVE</p><h1 tabindex="-1">思い出を、<br>ひとつずつ。</h1><p class="hero-copy">あの子のこと。あの日の冒険。<br>好きな物語を思い出しながら、ちいかわ検定に挑戦。<br>知らなかったことも、次の楽しみになる。</p><span class="small">全60問から毎回10問 / 登録不要 / 時間制限なし</span></div><div class="hero-sheet" aria-hidden="true"><div class="mini">ちいかわ検定 · 受検のしおり</div><div class="sheet-title">好き、を育てる。</div>${grass}<div class="circle">5級<small>ここから一歩</small></div><p class="small">一問ずつ、ゆっくり。<br>今日のあなたの「知ってる」を集めよう。</p></div></section>${state.run ? `<section class="resume"><p><b>${LEVELS[state.run.level].name} ${state.run.done ? "の結果があります" : `の途中です（${state.run.answers.filter((a) => a !== null).length} / 10問回答）`}</b><br>保存した内容から${state.run.done ? "結果を見返せます" : "再開できます"}。</p><button class="secondary" data-action="resume">${state.run.done ? "前回の結果を見る" : "続きから"}</button></section>` : ""}<div class="section-heading"><h2>今日は、何級に挑戦する？</h2><span class="small">どの級からでも受けられます。</span></div><section class="levels">${Object.entries(
      LEVELS,
    )
      .map(([key, l]) => {
        const rs = state.records.filter((r) => r.level === key),
          best = rs.length ? Math.max(...rs.map((r) => r.correct)) : null;
        return `<article class="level" style="--level:${l.color}"><span class="badge">${best === null ? "まだ見ぬ、あなたの記録" : best >= l.pass ? "合格済み · 最近の最高 " + best * 10 + "点" : "最近の最高 " + best * 10 + "点"}</span><div class="grade">${l.name}</div><h3>${l.title}</h3><p>${l.description}</p><small>10問中${l.pass}問正解で合格 · 約3分</small><button class="secondary" data-level="${key}">${l.name}に挑戦する <span aria-hidden="true">→</span></button></article>`;
      })
      .join(
        "",
      )}</section><aside class="warning"><b>ネタバレ注意</b><p>全ての級で、漫画・アニメ・映画本編のネタバレを含みます。問題と解説には、登場人物の行動や物語の展開が出てきます。</p></aside><section class="intro"><div><b>01 級を選んで挑戦</b><span>4択からひとつ。<br>焦らず、自分のペースで。</span></div><div><b>02 解説でおさらい</b><span>間違えた問題も、<br>物語を思い出すきっかけに。</span></div><div><b>03 合格証を集める</b><span>合格したら記念の一枚。<br>次の級にも挑戦しよう。</span></div></section><h2 class="records-heading">最近の記録</h2>${
...
function start() {
  ...
  state.run = {
    ...
    set: makeExam(chosenLevel, state.last[chosenLevel]),
  ...
  mount(
    `<div class="narrow"><section class="paper result"><p class="eyebrow">YOUR LITTLE ACHIEVEMENT</p>${grass}<h1 tabindex="-1">${s.passed ? "合格、おめでとう。" : "ここから、もう一歩。"}</h1><p>${l.name} · ${l.title}</p><div class="score">${s.points}<small> / 100点</small></div><p class="result-note">${s.correct} / 10問正解 · 合格ライン ${l.pass * 10}点<br>${s.passed ? "好きな世界のこと、またひとつ確かめられました。" : "知らなかったことも、今日からあなたの思い出に。解説を読んで、また挑戦してみよう。"}</p></section>${s.passed ? `<section class="certificate" id="certificate"><span class="eyebrow">CHIIKAWA FAN EXAM</span><h2>合 格 証</h2><p class="name" id="certificate-name">ちいかわ好きのあなた 様</p><p>ちいかわ検定 <b>${l.name}</b><br>${s.points}点で合格したことを、ここに記します。</p><div class="seal">${l.title}<br>合格</div><p>${new Intl.DateTimeFormat("ja-JP", { year: "numeric", month: "long", day: "numeric" }).format(new Date(r.date))}</p><p class="small">非公式ファン検定 · 本サイト独自の記念証です</p></section><label class="name-field">合格証に入れる名前（任意・20文字まで／保存しません）<input id="certificate-input" maxlength="20" placeholder="ちいかわ好きのあなた" autocomplete="off"></label><button class="secondary" data-action="print">合格証を印刷する</button>` : ""}<div class="actions"><button class="primary" data-level="${r.level}">別の10問に挑戦する</button><button class="secondary" data-action="home">ほかの級を選ぶ</button></div><div class="actions"><button class="secondary" data-action="share">結果をシェアする</button></div><div id="share-fallback"></div><h2 class="review-heading">答え合わせの時間</h2><p class="small">問題を開くと、あなたの回答・正解・解説が見られます。</p>${r.set
      .map((e, i) => {
        ...
        return `<details class="review" ${ok ? "" : "open"}><summary><span class="${ok ? "correct" : "wrong"}">${ok ? "正解" : "不正解"}</span> · Q${i + 1} ${esc(q.text)}</summary><p class="chosen">あなたの回答：${esc(q.options[r.answers[i]])}</p><p><b>正解：${esc(q.options[q.correct])}</b><br>${esc(q.explanation)}</p><a href="${src.url}" target="_blank" rel="noopener noreferrer">${src.label}で確かめる（別タブ）</a></details>`;
      })
      .join("")}${sister}</div>`,
  );
}
